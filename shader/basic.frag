...
uniform bool SmoothFlat;
uniform bool diffuse_switch;
...
void main(){
    ...
    if(normal_switch){
        pixelNormal = texture(TextureNormal, texCoord).rgb; // TexNormal
    ...
    if(diffuse_switch){
        color = vec4((ambient + diffuse + specular)*texture(TextureDiffuse, texCoord).rgb, 1.0f);
    }
    else{
        // Blinn-Phong Model:
